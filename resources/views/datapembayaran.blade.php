                                    ...
                                    <tbody>
                                        @foreach ($dataTransfer as $row)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($row->bukti_tf)
                                                        <a href="{{ asset('storage/Pembayaran/' . $row->bukti_tf) }}"
                                                            target="_blank">
                                                            <img src="{{ asset('storage/Pembayaran/' . $row->bukti_tf) }}"
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <form class="edit-data-form" method="POST"
                                                            action="{{ route('data-pembayaran.update', ['id' => $row->id]) }}">
                                                            @csrf
                                                            @method('PUT')
                                            ...
                                            <div class="modal fade" id="deleteModal{{ $row->id }}" tabindex="-1"
                                                role="dialog" aria-labelledby="deleteModal{{ $row->id }}Label"
                                                aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <form class="delete-data-form" method="POST"
                                                            action="{{ route('data-pembayaran.destroy', ['id' => $row->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <div class="modal-header">
                                                                <h5 class="modal-title"
                                                            ...
                                                            <div class="modal-body">
                                                                <p>Apa kamu ingin menghapus data ini?</p>
                                                            </div>
                                                            <div class="modal-footer">
                                                                <button type="submit"
